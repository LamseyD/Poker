...
var P, HCONF, ID_CONF, CALL_LEVEL, BET_LEVEL, POT_LEVEL, BALANCE;
...
function bot_get_preflop_bet(){
    ...
    if ((HUMAN_GOES_ALL_IN || HUMAN_WINS_AGAIN > 1) && (HCONF > 60 || RANKA == RANKB || RANKA > 13 || RANKB > 13)) {
        ...
        for (var i = 1; i < players.length; i++) {
            // Has she gone all-in ?
            // MAKING A STAND -> STAND UP AND NOT JOIN IN THE HAND
    ...
    //SUPER GOOD HAND. POT LEVEL IS HIGH WHICH LOWER THE CHANCE OF ALL IN AND NUMBER IN HAND IS OKAY
    //SO THERE MIGHT BE A BETTER HAND AVAILABLE
    if (HCONF > 99) {
        if (POT_LEVEL > 75) 
            return internal_what_do_x("60:ALLIN,10:BIG,20:MED,5:SMALL,5:CALL");
        if (NUM_IN_HAND < 4) 
            return internal_what_do_x("2:BIG,15:MED,33:SMALL,50:CALL");
        return internal_what_do_x("2:ALLIN,8:BIG,40:MED,40:SMALL,10:CALL");
    }
    //PRETTY GOOD HAND. POT LEVEL IS HIGH
    if (HCONF > 90) {
        if (POT_LEVEL > 50) 
            return internal_what_do_x("15:ALLIN,35:BIG,30:MED,15:SMALL,5:CALL");
        if (NUM_IN_HAND > 3) 
            return internal_what_do_x("5:ALLIN,15:BIG,35:MED,35:SMALL,10:CALL");
        return internal_what_do_x("2:ALLIN,6:BIG,15:MED,55:SMALL,22:CALL");
      }

    if (HCONF > 80) {
        if (POT_LEVEL > 50) {
            //ID CONF -> YOUR BALANCE CONFIDENCE
            //THE LOWER THE BALANCE THE MORE LIKELY TO GO ALL IN
            if (ID_CONF == "LO") 
                return internal_what_do_x("100:ALLIN");
            return internal_what_do_x("100:CALL");
        }
        return internal_what_do_x("5:ALLIN,15:BIG,15:MED,30:SMALL,35:CALL");
    }
    
    //HOLE CARDS ARE OKAY BUT NOT GOOD ENOUGH TO BET SO JUST CALL
    if (P.subtotal_bet > 0 && CALL_LEVEL < 40) {
        if (HCONF > 20 || RANKA > 10 || RANKB > 10) 
            return internal_what_do_x("5:SMALL,95:CALL");
    }
    
    //HOLE CARDS ARE BETTER
    if (HCONF > 70) {
        //POT IS BIG
        if (POT_LEVEL > 75) {
            //BOT BALANCE IS TOO LOW -> JUST ALL IN
            if (ID_CONF == "LO") 
                return internal_what_do_x("100:ALLIN");
            return internal_what_do_x("100:CALL");
        }
        ...
        if (POT_LEVEL > 50) {
            if (ID_CONF == "LO") 
                return internal_what_do_x("50:ALLIN,50:BIG");
            return internal_what_do_x("100:CALL");
        }
        //POT IS LOW ENOUGH TO CALL OR BET AND TOO MANY PEOPLE STILL IN
        if (NUM_IN_HAND > 3)
            return internal_what_do_x("5:ALLIN,15:BIG,30:MED,30:SMALL,20:CALL");
        return internal_what_do_x("2:ALLIN,7:BIG,35:MED,36:SMALL,20:CALL");
    }
    
    //HOLE CARDS ARE PRETTY OKAY
    if (HCONF > 60) {
        if (POT_LEVEL > 75) {
            if (ID_CONF == "LO") 
                return internal_what_do_x("100:ALLIN");
        ...
        if (POT_LEVEL > 50) {
            if (ID_CONF == "LO") 
                return internal_what_do_x("75:ALLIN,25:BIG");
            if (CALL_LEVEL < 70) 
                return CALL;
            return internal_what_do_x("65:CALL");
        }
        if (NUM_IN_HAND > 3) 
            return internal_what_do_x("3:ALLIN,17:BIG,30:MED,30:SMALL,20:CALL");
    ...
    if (HCONF > 40) {
        ...
        if (BET_LEVEL > 30) {
            if (CALL_LEVEL < 30) 
                return CALL;
            if (ID_CONF == "LO") 
                return internal_what_do_x("24:CALL");
    ...
    if (HCONF > 30) {
        ...
        if (BET_LEVEL > 30) {
            if (CALL_LEVEL < 30) 
                return internal_what_do_x("15:SMALL,85:CALL");
            if (ID_CONF == "LO") 
                return internal_what_do_x("1:CALL");
    ...
    if (CALL_LEVEL > 5) {
        if (ID_CONF == "LO") 
            return internal_what_do_x("1:BIG,15:CALL");
        return internal_what_do_x("35:CALL");
    }
    if (ID_CONF == "LO") 
        return internal_what_do_x("1:ALLIN,79:CALL");
    return CALL;
...
function bot_get_postflop_bet () {
    ...
    if (VERDICT == "" && FLUSH["num_needed"] < 1) { // look for full house, etc.
        ...
        if (num_mine > 1) 
            ...
            var rank = 0;
            if (P.carda.substring(0, 1) == FLUSH["suit"]) 
                rank = RANKA;
            else rank = RANKB;
            if (rank < 11) 
                VERDICT = "GOOD"; // 12???????
            ...
            VERDICT = "MAYBE"; // could look @ board & decide if person was tryin' for flush...FACTOR: ANALYZE BETTING PATTERNS!...
    }

    //STRAIGHT TEST
    if (VERDICT == "" && STRAIGHT["num_needed"] < 1) { // look for flush, etc.
        if (STRAIGHT["num_mine"] > 0) 
            VERDICT = "GREAT";
        else 
            VERDICT = "PLAY BOARD";
...
function internal_get_hole_ranking(){
    ...
    var r_rank_a = internal_my_make_readable_rank(RANK_A);
    var r_rank_b = internal_my_make_readable_rank(RANK_B);
    var suited = "";
    //GET_SUIT IS FROM HANDS.JS
    if (get_suit(P.carda) == get_suit(P.cardb))
        suited = "s"; // same suit
    var h = "";
    if (RANK_A == RANK_B)
        h = "" + r_rank_a + "" + r_rank_b;
...
function internal_my_make_readable_rank (r) {
    //make_readable_rank is from main.js
...
function internal_setup(){
    ...
    //current_bet_amount is from main.js 
    //in js apparently you can access global variables from another file with no mention
    //as long as they're in the same folder?
    CALL = current_bet_amount - P.sub_bet;

    //GET_RANK IS FROM HANDS.JS

    RANK_A = get_rank(P.carda);
    RANK_B = get_rank(P.cardb);
    //HCONF -> hole confidence level
    ...
    BALANCE = P.balance;
    var total_balance = get_pot_size();
    ...
    //Confidence level
    ID_CONF = "MID";
    var average_balance = total_balance / NUM_IN_GAME;
    if (BALANCE < average_balance / 2) 
        ID_CONF = "LOW";
    if (BALANCE > average_balance * 1.5) 
        ID_CONF = "HIGH";
    //SET BET LEVELS HERE
    //NO MIN RAISE CONSIDERATION YET
...
function internal_what_do_x (q, r) {
    ...
    while (1) {
        var a = q.indexOf(":");
        //start from a, find first occurent of ,
        var b = q.indexOf(",", a);
        if (a < 0 || b < 0) 
            return FOLD;
...
function internal_get_pot_level () {
    var p = get_pot_size();
    var b = players[current_bettor_index].balance;
    if (p > 0.5 * b)
        //if pot > half of current bot's balance
        return 100;
    //if pot > a quarter of current bot's balance
    if (p > 0.25 * b)
        return 51;
    return 1;
}